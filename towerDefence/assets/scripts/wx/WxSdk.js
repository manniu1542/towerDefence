...
export default class WxSdk {
	...
	static createUserInfoButton(successCallback) {
        ...
        var fra = cc.view.getFrameSize();
        var scaleX = fra.width / vis.width;
        var scaleY = fra.height / vis.height;
        var picture = cc.p(1440, 174); //大小
        var width = picture.x * scaleX;
        var height = picture.y * scaleY;
        var pos = cc.size(0, -513); //中间 位置
        var top = (fra.height / 2 - pos.height * scaleY) - height / 2;
        var left = (fra.width / 2 + pos.width * scaleX) - width / 2;
        cc.loader.loadRes('texture/login', function (err, res) {
            ...
            button.onTap(function (res) {
                console.log('进入授权过程中', res, 'button', button);
                if (res.userInfo) {
                    //button.hide();
                    button.destroy();
                    WeiXinUtils.userInfo = res.userInfo;
                }
                successCallback(res);
            });
            return button;
